...
def test_misc1():
  """
  Description:
    Tests what happens when the branch origin diverted, and local is behind.
...
def test_misc2():
  """
  Description:
    Tests what happens when the branch origin diverted, and local is caught up and ahead.
...
def test_no_main():
  ...
  run_test(
    " && ".join(
      [
        "git init && git checkout -b branch1",
        commit("A", now + sec * 1),
        commit("B", now + sec * 2),
      ]
    ),
