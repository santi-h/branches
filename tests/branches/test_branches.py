...
def test_local_branches_order():
  ...
  for test_case in test_cases:
    assert local_branches_order(**test_case[0]) == test_case[1]


def test_branches_ahead_shas_to_refs():
  assert branches_ahead_shas_to_refs(
    {
      "b2": ["38089"],
...
def test_base_branches_from_branches_ahead_refs():
  assert base_branches_from_branches_ahead_refs(
    [
      ("b2", ["b2"]),
      ("b5", ["b5~1", "b5"]),
      ("b8", ["b5~1", "b8"]),
